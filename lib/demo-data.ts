...
export const DEMO_REVENUE: RevenuePoint[] = Array.from(
  ...
  (_, index) => {
    ...
    const timestamp = DEMO_END_DATE - daysBeforeEnd * DAYS_MS;
    ...
    const wave = Math.round(
      Math.sin(chartIndex / 4.2) * 170 + Math.cos(chartIndex / 9.5) * 85,
    );
...
export const DEMO_TICKETS: TicketRecord[] = [
  ...
  {
    ticketId: 4_804,
    subject: "Customer cannot update their biling address",
    team: "Billing",
    status: "closed",
...
export function getDemoRevenue(range: RevenueRange): RevenueResult {
  ...
  const previousRows = DEMO_REVENUE.slice(-(days * 2), -days);
  ...
  return {
    ...
    changePct:
      previousTotal === 0
        ? 0
        : round(
            ((comparisonTotal - previousTotal) / Math.abs(previousTotal)) * 100,
            1,
          ),
    ...
    peakDay: peakDay
      ...
      : null,
    refundCount: series.filter((point) => point.refunded || point.amount < 0)
      .length,
    dip,
    series,
  };

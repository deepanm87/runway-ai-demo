...
import { DEEP_AVATAR_ID, DEEP_IMAGE } from "@/lib/avatar"
...
export function AgentCard({
  ...
  function showCallError(callError: Error) {
    ...
    const message = callError.message.toLowerCase()
    if (message.includes("not enough credits")) {
      setError("Your Runway account does not have enough credits to start a call.")
      return
    }
    if (message.includes("runwayml_api_secret")) {
      setError("Add RUNWAYML_API_SECRET to .env.local and restart the dev server.")
      return
    }
    setError("The call could not start. Check the local Runway setup and try again.")
  }
  ...
  return (
    <>
      <aside
        ...
        aria-label="Deep customer support agent"
        aria-hidden={!open}
        inert={!open}
      >
          ...
          {session ? (
            ...
            <div className="flex min-h-full flex-col justify-center p-6">
              <div className="flex items-center gap-4">
                <Image 
                  src={DEEP_IMAGE}
                  alt="Deep, Northstar customer support"
                  width={72}
                  height={72}
                  className="size-18 rounded-2xl object-cover object-top"
                  preload
                />
                <div>
                  ...
                  <h2 className="mt-1 text-2xl font-semibold tracking-wider">
                    How can I help?
                  </h2>
                </div>
              </div>
              <div className="mt-5 rounded-2xl border border-white/10 bg-white/4 p-3.5">
                <p className="font-mono text-[0.58rem] uppercase tracking-[0.14em] text-white/38">
                  What Deep can do
                </p>
                <p className="mt-2 text-xs leading-5 text-white/62">
                  Navigate and highlight the UI, show demo dashboard insights, change revenue ranges, filter tickets, open panels, and create or update tickets.
                </p>
              </div>
              <div className="mt-5">
                <Button
      ...
      <button
        id="deep-widget-open"
        data-avatar-target="deep-widget-open"
        type="button"
        className={cn(
